...
public class Account extends Controller {
	...
	public Account( final PlayAuthenticate auth, final UserProvider userProvider,
			final MyUsernamePasswordAuthProvider myUsrPaswProvider, final FormFactory formFactory, final MessagesApi msg ) {
		this.auth = auth;
	...
	public Result doLink() {
		...
		if ( filledForm.hasErrors() )
			...
			if ( link )
				flash( Application.FLASH_MESSAGE_KEY, this.msg.preferred( request() ).at( "playauthenticate.accounts.link.success" ) );
	...
	public Result doMerge() {
		...
		if ( filledForm.hasErrors() )
			...
			if ( merge )
				flash( Application.FLASH_MESSAGE_KEY, this.msg.preferred( request() ).at( "playauthenticate.accounts.merge.success" ) );
